...
/************************************
Methods of class FileAndFolderSelectionDialog:
************************************/

std::string FileAndFolderSelectionDialog::getCurrentPath(void) const
    {
...
bool FileAndFolderSelectionDialog::readDirectory(void)
    {
...
void FileAndFolderSelectionDialog::updateActionButtons(void)
    {
...
void FileAndFolderSelectionDialog::setSelectedPathButton(int newSelectedPathButton)
    {
    /* Get the style sheet: */
...
void FileAndFolderSelectionDialog::pathButtonSelectedCallback(Button::SelectCallbackData* cbData)
    {
...
void FileAndFolderSelectionDialog::listItemValueChangedCallback(Misc::CallbackData* cbData)
    {
    /* Update the action buttons */
    updateActionButtons();
    }

void FileAndFolderSelectionDialog::listItemSelectedCallback(ListBox::ItemSelectedCallbackData* cbData)
    {
    ...
    if(item[item.size()-1]=='/')
        enterButtonSelectedCallback(cbData);
    else
        pickButtonSelectedCallback(cbData);
    }

void FileAndFolderSelectionDialog::filterListValueChangedCallback(DropdownBox::ValueChangedCallbackData* cbData)
    {
...
void FileAndFolderSelectionDialog::pickButtonSelectedCallback(Misc::CallbackData* cbData)
    {
    ...
    if(selectedItem>=0)
        ...
        if (selectedFileName[selectedFileName.size()-1]=='/')
            selectedFileName.resize(selectedFileName.size()-1);

        /* Call the OK callbacks: */
        OKCallbackData okData(this,selectedFileName);
        okCallbacks.call(&okData);
        }
    }

void FileAndFolderSelectionDialog::enterButtonSelectedCallback(Misc::CallbackData *cbData)
    {
    /* get the selected directory entry: */
    int selectedItem=fileList->getListBox()->getSelectedItem();
    if(selectedItem>=0)
        ...
        if(item[item.size()-1]=='/')
            ...
            pathButton->getSelectCallbacks().add(this,&FileAndFolderSelectionDialog::pathButtonSelectedCallback);
...
void FileAndFolderSelectionDialog::cancelButtonSelectedCallback(Misc::CallbackData* cbData)
    {
...
FileAndFolderSelectionDialog::FileAndFolderSelectionDialog(WidgetManager* widgetManager,const char* titleString,const char* initialDirectory,const char* sFileNameFilters,Comm::MulticastPipe* sPipe)
    :PopupWindow("FileAndFolderSelectionDialogPopup",widgetManager,titleString),
     pipe(sPipe),
    ...
    RowColumn* fileSelectionDialog=new RowColumn("FileAndFolderSelectionDialog",this,false);
    ...
    rootButton->getSelectCallbacks().add(this,&FileAndFolderSelectionDialog::pathButtonSelectedCallback);
    ...
    filterList->getValueChangedCallbacks().add(this,&FileAndFolderSelectionDialog::filterListValueChangedCallback);
    }
    ...
    /* Create the cancel command button: */
    enterButton=new Button("Enter",commandButtonBox,"-");
    enterButton->getSelectCallbacks().add(this,&FileAndFolderSelectionDialog::enterButtonSelectedCallback);
    pickButton=new Button("Pick",commandButtonBox,"-");
    pickButton->getSelectCallbacks().add(this,&FileAndFolderSelectionDialog::pickButtonSelectedCallback);
    Button* cancelButton=new Button("Cancel",commandButtonBox,"Cancel");
    cancelButton->getSelectCallbacks().add(this,&FileAndFolderSelectionDialog::cancelButtonSelectedCallback);
...
void FileAndFolderSelectionDialog::addFileNameFilters(const char* newFileNameFilters)
    {
...
void FileAndFolderSelectionDialog::defaultCloseCallback(FileAndFolderSelectionDialog::CallbackData* cbData)
    {
